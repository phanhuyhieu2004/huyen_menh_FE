    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10 border-b border-gold/10 pb-8">
      ...
      <div class="bg-black/40 border border-gold/20 rounded-2xl p-1.5 flex items-center gap-1 backdrop-blur-2xl shadow-[0_0_20px_rgba(255,184,0,0.1)]">
        <button v-for="range in ranges" :key="range.id"
                @click.stop="changeRange(range.id)"
...
const fetchStats = async (days = 30) => {
  ...
  console.group("📡 [THỐNG KÊ] Đang triệu hồi từ vũ trụ (Fetch API)");
  console.log("Số ngày yêu cầu:", days);
  console.log("Endpoint dự kiến: /api/admin/stats?days=" + days);
  try {
    ...
    console.error("Lỗi khi kết nối với tinh linh:", error);
...
const changeRange = (rangeId) => {
  alert("🚀 [XÁC NHẬN] Sếp vừa nhấn: " + rangeId);
  console.group("🔘 [HÀNH ĐỘNG] Chuyển đổi chu kỳ");
  console.log("Mốc thời gian vừa chọn:", rangeId);
  selectedRange.value = rangeId;
  const daysMap = { '7d': 7, '30d': 30, '90d': 90 };
  const targetDays = daysMap[rangeId] || 30;
  console.log("=> Số ngày tương ứng sẽ gửi lên BE:", targetDays);
