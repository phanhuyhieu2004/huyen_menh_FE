<template>
  <div class="reading-page min-h-screen pt-24 pb-12 px-4 relative flex flex-col items-center">
    ...
    <div class="text-center mb-10 animate-fade-in relative z-10">
        <h2 class="text-2xl md:text-3xl font-serif font-bold text-transparent bg-clip-text bg-gradient-to-r from-gold-light to-gold-dark tracking-[0.4em] glowing-text-gold uppercase">
            XEM QUẺ MỆNH
        </h2>
    ...
    <div class="w-full max-w-2xl mb-12 animate-slide-up-fade relative z-10">
        ...
        <div class="relative group">
            <button @click="showInfoModal = true" class="absolute left-3 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full flex items-center justify-center border border-gold/40 text-gold bg-gold/5 hover:bg-gold/20 hover:border-gold hover:shadow-[0_0_15px_rgba(255,184,0,0.6)] transition-all duration-300 group/info z-10" title="Đạo Tắc Thỉnh Giáo">
                <div class="absolute inset-0 rounded-full bg-gold/20 blur-md animate-pulse group-hover/info:bg-gold/40 transition-colors pointer-events-none"></div>
                <svg class="w-4 h-4 relative z-10 shake-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            ...
            <input
                ...
                class="w-full bg-black/40 border border-gold/30 rounded-full py-3 pl-14 pr-16 text-white placeholder-white/40 focus:outline-none focus:border-gold/60 focus:bg-black/60 transition-all duration-300 shadow-[0_4px_20px_rgba(0,0,0,0.5)]"
    ...
    <!-- Modal Hướng Dẫn Thỉnh Giáo (Dùng chung InfoModal) -->
    <InfoModal v-model:show="showInfoModal" title="Đạo Tắc Thỉnh Giáo">
        <div class="space-y-6 text-white/80 text-sm font-sans leading-relaxed">
            <p class="flex gap-4">
                <i class="fi fi-rr-comment-quote text-gold text-xl mt-0.5 drop-shadow-[0_0_8px_rgba(255,184,0,0.3)]"></i>
                <span><strong class="text-gold-light block mb-1">Thành tâm đặt câu hỏi:</strong> Lời thỉnh cầu cần có nội dung và mục đích cụ thể. Vui lòng không dùng các ký tự vô nghĩa, trống rỗng hay câu chữ thiếu cẩn trọng khi ra quyết định.</span>
            </p>
            <p class="flex gap-4">
                <i class="fi fi-rr-time-forward text-gold text-xl mt-0.5 drop-shadow-[0_0_8px_rgba(255,184,0,0.3)]"></i>
                <span><strong class="text-gold-light block mb-1">Quy luật Tương Lai:</strong> Nếu bạn hỏi về một sự kiện sắp diễn ra, Thầy sẽ dựa vào <em>Năng Lượng của chánh niệm Hiện Tại</em> làm nền tảng. Tương lai của bạn luôn có thể được chuyển hóa thông qua hành động lúc này.</span>
            </p>
            <p class="flex gap-4">
                <i class="fi fi-rr-link-alt text-gold text-xl mt-0.5 drop-shadow-[0_0_8px_rgba(255,184,0,0.3)]"></i>
                <span><strong class="text-gold-light block mb-1">Sự thấu cảm của Thầy:</strong> Theo lý số tùy duyên, ngay khi gieo quẻ, Thầy đã kết nối được với thông tin bản mệnh của bạn (Ngày Sinh, Cung Hoàng Đạo) cùng đồ thị năng lượng thực tại để luận giải dành riêng cho bạn.</span>
            </p>
            <p class="flex gap-3 text-white/50 italic text-xs mt-8 pt-6 border-t border-white/5">
                <i class="fi fi-rr-shield-interrogation text-gold/40 mt-0.5"></i>
                Xin tuyệt đối không thỉnh giác những vấn đề xâm phạm quyền riêng tư của cá nhân thứ ba hoặc những việc trái luân thường đạo lý.
            </p>
        </div>
    </InfoModal>
