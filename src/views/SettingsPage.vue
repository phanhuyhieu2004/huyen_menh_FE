<template>
  <div class="min-h-[80vh] flex flex-col md:flex-row gap-6 p-4 md:p-8 animate-fade-in relative z-10">
    ...
    <Teleport to="body">
      <div v-show="!editingItem" class="md:hidden fixed bottom-10 right-10 z-[999]">
        ...
        <transition name="tab-slide" mode="out-in">
          <div :key="activeTab" class="relative z-10">
            ...
            <div v-else-if="activeTab === 'stats'" class="space-y-10 animate-fade-in relative">
              ...
              <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-gold/10 pb-8">
                ...
                <div class="bg-black/40 border border-gold/20 rounded-2xl p-1.5 flex items-center gap-1 backdrop-blur-2xl">
                  <button v-for="range in ranges" :key="range.id" @click="changeRange(range.id)"
            ...
            <div v-else-if="activeTab === 'data'" class="space-y-8 animate-fade-in">
              ...
              <Teleport to="body">
                <div class="fixed bottom-10 right-6 z-[999] md:hidden flex flex-col items-center animate-slide-up">
...
const quickStats = computed(() => [
  ...
  { label: 'Luồng Năng Lượng', value: stats.value?.totalEnergyFlow?.toLocaleString() || '0', icon: 'fi fi-rr-sparkles', trend: '+18%' },
  { label: 'Độ Tương Thích', value: (stats.value?.averageCompatibility || 0) + '%', icon: 'fi fi-rr-bolt', trend: '+2%' }
]);
...
const fetchStats = async (days = 30) => {
  ...
  console.log("📡 [FETCH] Gọi API stats (Settings) với số ngày:", days);
  try {
...
const changeRange = (rangeId) => {
  console.log("🚀 [CLICK] Thay đổi chu kỳ sang:", rangeId);
  selectedRange.value = rangeId;
  const daysMap = { '7d': 7, '30d': 30, '90d': 90 };
  fetchStats(daysMap[rangeId] || 30);
};
...
watch(activeTab, (newTab) => {
    ...
    if (newTab === 'stats' && authStore.user?.role === 'admin') {
      const daysMap = { '7d': 7, '30d': 30, '90d': 90 };
      fetchStats(daysMap[selectedRange.value] || 30);
    }
});
...
const fetchUsers = async () => {
    try {
        ...
        const data = res.data;
        filteredUsers.value = data.content || data || [];
...
const fetchLibraryItems = async () => {
    try {
        ...
        const data = res.data;
        filteredLibraryItems.value = data.content || data || [];
        dataTotalPages.value = data.page?.totalPages ?? data.totalPages ?? 1;
        dataTotalElements.value = data.page?.totalElements ?? data.totalElements ?? 0;
    } catch (e) { 
        console.error(e);
